 ...
 */
class RandomFullMoveSearchDoublingTest : AbstractRandomFullMoveSearchTest() {
    ...
    fun testPerformance2() {
        ...
        for (i in 1..15000) { // Warm up
            RandomFullMoveSearchDoubling(board, Player.MCTS, dices).findAll()
        }
    ...
    fun `Test single checker move`() {
        ...
        assertRandomMoveFound(movesSequence)
    }
    ...
    @Test
    fun `Test one locked checker sequence (not full)`() {
        player1Board.put(24, 1)
        player2Board.put(toOpponentsIndex(16), 2)
        dice = dice(2, 2)

        val movesSequence = listOf(
                movesSequence(move(24, 22), move(22, 20), move(20, 18))
        )
        assertRandomMoveFound(movesSequence)
    }
